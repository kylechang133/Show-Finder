...
def select_show_button():
    ...
    try:
        ...
        for i in range(0, len(SEASON)):
            display_season_text(season_text_box, SEASON[i]['number'], format_season_name(SEASON[i]), n)
            n += 1
    except:
...
def select_season_button():
    ...
    try:
        ...
        n = 1
        episode_text_box.insert(END, f"Episodes of {SHOW_NUMBER['show']['name']} S{season + 1}:\n")
...
show_text_box = Text(show_results_frame, width = 80, height = 20, yscrollcommand = show_scrollbar.set)
...
season_text_box = Text(season_results_frame, width = 80, height = 20, yscrollcommand = season_scrollbar.set)
...
episode_text_box = Text(episode_results_frame, width = 80, height = 20, yscrollcommand = episode_scrollbar.set)
